        ...
        <ul class="nav nav-tabs mb-4" id="financeTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="dashboard-tab" data-bs-toggle="tab" data-bs-target="#dashboard"
                    type="button" role="tab">Painel</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="lancamentos-tab" data-bs-toggle="tab" data-bs-target="#lancamentos"
                    type="button" role="tab">Lançamentos</button>
            </li>
        </ul>
        ...
        <div class="tab-content" id="financeTabContent">
            <div class="tab-pane fade show active" id="dashboard" role="tabpanel">
                ...
                <div class="row text-center">
                    ...
                    {{-- Componente de classe com parâmetro: mensagens da session --}}
                    @if(session('mensagem'))
                        <x-mensagem-alerta tipo="success" :mensagem="session('mensagem')" />
                    @endif
                </div>
                ...
                <div class="row">
                    <div class="col-md-8 mx-auto">
                        <div class="card p-4">
                            ...
                            {{-- Mensagens dinâmicas --}}
                            @if(session('success'))
                                <x-mensagem tipo="success">{{ session('success') }}</x-mensagem>
                            @endif
                            @if($errors->any())
                                <x-mensagem tipo="danger">{{ $errors->first() }}</x-mensagem>
                            @endif
                            ...
                            <form id="formTransacao">
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        <x-select id="tipo" name="tipo" label="Tipo" :options="$tipos"
                                            :selected="old('tipo')" />
                                    </div>
                                    <div class="col-md-6">
                                        <x-select id="categoria" name="categoria" label="Categoria"
                                            :options="$categorias" :selected="old('categoria')" />
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        <x-select id="forma_pagamento" name="forma_pagamento" label="Forma de Pagamento"
                                            :options="$formasPagamento" :selected="old('forma_pagamento')" />
                                    </div>
                                    <div class="col-md-6 d-flex align-items-end">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="recorrente"
                                                name="recorrente" value="1" {{ old('recorrente') ? 'checked' : '' }}>
                                            <label class="form-check-label" for="recorrente">Recorrente</label>
                                        </div>
                                    </div>
                                </div>

                                <button type="button" class="btn btn-primary" onclick="registrarTransacao()">Salvar
                                    Transação</button>
                            </form>
                        ...
                        <div class="card p-4 mt-4">
                            ...
                            <form action="{{ route('importar.transacoes') }}" method="POST"
